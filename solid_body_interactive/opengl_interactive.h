...
protected:
    bool Handle_Click(int button,int state,int x,int y,bool ctrl_pressed,bool shift_pressed)
    {
        ...
        switch(button){
            case GLUT_LEFT_BUTTON:
                opengl_world.ctrl_was_pressed=ctrl_pressed!=0;
                opengl_world.shift_was_pressed=shift_pressed!=0;
                if(state==GLUT_UP){
                    opengl_world.drag_current_selection=false;
                    if(current_selection){
                        ...
                        ((OPENGL_COMPONENT_RIGID_BODY_COLLECTION_3D<float> *)obj)->rigid_body_collection_simulated->Rigid_Body(real_selection->body_id).V()=object_velocity;
                        current_selection=0;
                        last_mouse_location_init=false;
                        last_mouse_location=TV(0,0,0);}}
                if(ctrl_pressed){
                    if(state==GLUT_DOWN){
                        current_selection=0;
                        ...
                        if(hits<0) hits=opengl_world.Get_Number_Of_Valid_Hits(hits,selectBuf,buff_size); // if had buffer overflow, get number of valid hit records
                        Process_Hits(hits,selectBuf);

                        GLdouble modelview[16];
                        GLdouble projection[16];

                        opengl_world.Prepare_For_Target_XY_Drag();        
                        ...
                        if(current_selection){
                            ...
                            gluUnProject((GLdouble)0,(GLdouble)0,(GLdouble)1,modelview,projection,viewport,&project_x,&project_y,&project_z);
                            PLANE<T> view_plane(TV(project_x,project_y,project_z).Normalized(),((OPENGL_COMPONENT_RIGID_BODY_COLLECTION_3D<float> *)obj)->rigid_body_collection.Rigid_Body(real_selection->body_id).X());
                            if(INTERSECTION::Intersects(ray,view_plane)){
                                opengl_world.object_location=ray.Point(ray.t_max);
                                object_delta=opengl_world.object_location-((OPENGL_COMPONENT_RIGID_BODY_COLLECTION_3D<float> *)obj)->rigid_body_collection.Rigid_Body(real_selection->body_id).X().Normalized();}
                            current_selection=current_selection;
                            OPENGL_WORLD::Singleton()->Bind_Key("f",Display_Follow_Object_CB("Select All"));}
                        else OPENGL_WORLD::Singleton()->Unbind_Keys("f");

                        since_last_drag=std::clock();
                        object_velocity=TV(0,0,0);
                        last_mouse_location_init=false;
                        opengl_world.drag_current_selection=true;}}}
						*/
        return ctrl_pressed;
    }
    ...
    {
			/*
        if(opengl_world.drag_current_selection){
            GLint viewport[4];
            GLdouble modelview[16];
            GLdouble projection[16];

            OPENGL_WORLD::Singleton()->Prepare_For_Target_XY_Drag();        
            ...
            if(current_selection){ 
                ...
                if(INTERSECTION::Intersects(ray,view_plane)){
                    ...
                    object_delta=opengl_world.object_location-((OPENGL_COMPONENT_RIGID_BODY_COLLECTION_3D<float> *)obj)->rigid_body_collection.Rigid_Body(real_selection->body_id).X().Normalized();
                    now=std::clock();
                    if(last_mouse_location_init){
                        object_velocity=(opengl_world.object_location-last_mouse_location)/(((float)now-since_last_drag)/CLOCKS_PER_SEC);} 
                    else object_velocity=object_delta/(((float)now-since_last_drag)/CLOCKS_PER_SEC);
                    last_mouse_location=opengl_world.object_location;
                    last_mouse_location_init=true;
                    since_last_drag=now;}}
            glFlush();}
			*/
